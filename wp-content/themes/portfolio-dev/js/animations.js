...
(function () {
	...
	/**
	 * Sketchbook Section Reveal
	 * - Cover image fades in and slides up on scroll
	 */
	function initSketchbookReveal() {
		const sketchbookSection = document.querySelector('.sketchbook-section');
		const sketchbookCover = document.querySelector('.sketchbook-cover');

		if (!sketchbookSection || !sketchbookCover) return;

		gsap.set(sketchbookCover, { opacity: 0, y: 40 });
		gsap.to(sketchbookCover, {
			opacity: 1,
			y: 0,
			duration: 1,
			ease: 'power2.out',
