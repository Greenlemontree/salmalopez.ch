...
function portfolio_salma_scripts() {
	...
	if ( is_front_page() ) {
		...
		// Sketchbook interaction script
		wp_enqueue_script(
			'portfolio-salma-sketchbook',
			get_template_directory_uri() . '/js/sketchbook.js',
			array(),
			_S_VERSION,
			true
		);
	...
	// Work page and single project scripts
	if ( is_page( 'work' ) || is_singular( 'project' ) ) {
		// GSAP Core
		wp_enqueue_script(
			'gsap',
			'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/gsap.min.js',
		...
		// Work carousel script (only on Work page)
		if ( is_page( 'work' ) ) {
			wp_enqueue_script(
				'portfolio-salma-work-carousel',
				get_template_directory_uri() . '/js/work-carousel.js',
				array( 'gsap', 'gsap-scrolltrigger' ),
				_S_VERSION,
				true
			);
		...
		if ( is_singular( 'project' ) ) {
			// Comparison slider
			wp_enqueue_script(
				'portfolio-salma-comparison-slider',
				get_template_directory_uri() . '/js/comparison-slider.js',
				array(),
				_S_VERSION,
				true
			);
			...
			// BTS carousel
			wp_enqueue_script(
				'portfolio-salma-bts-carousel',
				get_template_directory_uri() . '/js/bts-carousel.js',
				array(),
				_S_VERSION,
				true
			);
			...
			// Film extracts & video cover
			wp_enqueue_script(
				'portfolio-salma-film-extracts',
				get_template_directory_uri() . '/js/film-extracts.js',
				array( 'gsap', 'gsap-scrolltrigger' ),
				_S_VERSION,
				true
			);
...
require get_template_directory() . '/inc/settings/maintenance.php';
